...
export default function WaveformMatchPage() {
  ...
  const [results, setResults] = useState<
    { round: number; detune: number; answer: number; score: number }[]
  >([]);
  ...
  const handleSlider = (val: number) => {
    ...
    if (canvasRef2.current) {
      drawWaveform(
        canvasRef2.current,
        baseFreq,
        val > 0 ? "var(--ios-orange)" : "var(--ios-blue)",
        "Your Match",
        val,
      );
    }
  ...
  const submit = () => {
    ...
    const newResults = [
      ...results,
      { round, detune: detuneCents, answer: sliderCents, score: roundScore },
    ];
  ...
  useEffect(() => {
    ...
    if (!recordedRef.current) {
      ...
      recordResult({
        ...
        accuracy:
          results.length > 0
            ? results.filter((r) => Math.abs(r.detune - r.answer) <= 20).length / results.length
            : 0,
        rounds: Math.max(results.length, 1),
        date: new Date().toISOString(),
        timeMs: Math.max(results.length, 1) * 5000,
      });
    }
  }, [phase, recordResult]);
        ...
        <div style={{ marginBottom: 20 }}>
          ...
          >
            ...
            <span
              style={{
                fontWeight: 600,
                color: Math.abs(sliderCents) < 5 ? "var(--ios-green)" : "var(--ios-label)",
              }}
            >
              {sliderCents > 0 ? "+" : ""}
              {sliderCents}¢
        ...
        <AnimatePresence>
          {showResult && (
            ...
            >
              ...
              >
                {lastRoundScore >= 70 ? "✓ Nice!" : lastRoundScore >= 40 ? "~ Close" : "✗ Off"} —{" "}
                {lastRoundScore}pts
              </div>
              <div style={{ fontSize: 12, color: "var(--ios-label3)", marginTop: 4 }}>
                Target: {detuneCents > 0 ? "+" : ""}
                {detuneCents}¢ · Your answer: {sliderCents > 0 ? "+" : ""}
                {sliderCents}¢
              </div>
            </motion.div>
          )}
