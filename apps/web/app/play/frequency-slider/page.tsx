...
export default function FrequencySliderPage() {
  ...
  const [results, setResults] = useState<
    { round: number; freq: number; answer: number; centsOff: number; points: number }[]
  >([]);
  ...
  const handleSubmit = () => {
    ...
    setResults((r) => [
      ...r,
      { round: roundRef.current, freq: targetFreq, answer: answerFreq, centsOff, points },
    ]);
    setPhase("reveal");
  };
  ...
  useEffect(() => {
    ...
    if (!recordedRef.current) {
      ...
      recordResult({
        ...
        accuracy:
          results.length > 0
            ? results.filter((r) => Math.abs(r.centsOff) <= 50).length / results.length
            : 0,
        rounds: TOTAL_ROUNDS,
    ...
    return (
      <StudioResults
        ...
        stats={[
          ...
          { value: `${results.length ? Math.round(results.reduce((s, r) => s + Math.abs(r.centsOff), 0) / results.length) : 0}¢`, label: "AVG ERROR" },
          { value: results.length ? Math.max(...results.map((r) => r.points)) : 0, label: "BEST ROUND" },
        ]}
          ...
          {submitted && (
            ...
            >
              <div style={{ fontSize: 13, color: "var(--ios-label3)" }}>
                Target:{" "}
                <span style={{ fontWeight: 700, color: "var(--ios-label)" }}>
              ...
              <div
                style={{
                  ...
                  fontWeight: 700,
                  color:
                    Math.abs(1200 * Math.log2(answerFreq / targetFreq)) <= 15
                      ? "var(--ios-green)"
                      : "var(--ios-orange)",
                }}
              >
