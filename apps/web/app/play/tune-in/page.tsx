    ...
    return (
      <div className="min-h-screen px-4 pt-10">
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="mx-auto max-w-md text-center">
          ...
          <p className="mt-2 text-zinc-500">Hit the target note with your voice</p>
          <div className="mt-8 flex items-center justify-center gap-3">
            ...
            <button onClick={() => setUseMidi(true)} className={`rounded-full px-5 py-2 text-sm font-medium transition-all ${useMidi ? 'text-white' : 'bg-white/5 text-zinc-500 hover:bg-white/10'}`} style={useMidi ? { background: ACCENT } : {}}>🎹 Listen & Match</button>
          </div>
      ...
      <div className="mx-auto max-w-md">
        ...
        <div className="mt-8 relative">
          ...
          <div className="relative h-3 overflow-hidden rounded-full bg-white/5">
            ...
            <motion.div className="absolute top-1/2 -translate-y-1/2 h-4 w-4 rounded-full border-2" style={{ left: `${50 + (centsOff / 50) * 45}%`, borderColor: centsColor, backgroundColor: `${centsColor}40`, boxShadow: `0 0 8px ${centsColor}60` }} transition={{ type: 'spring', stiffness: 300, damping: 30 }} />
          </div>
        ...
        <div className="mt-8 flex flex-col items-center gap-3">
          <motion.button onClick={() => playTone(targetFreq, 0.8)} whileTap={{ scale: 0.92 }} className="flex h-16 w-16 items-center justify-center rounded-2xl bg-white/5 border border-white/10 text-2xl hover:bg-white/10 transition-all">
            🔊
          </motion.button>
          <p className="text-xs text-zinc-600">Tap to hear target</p>

          <button onClick={handleGiveUp} className="mt-4 text-sm text-zinc-600 hover:text-zinc-400 transition-colors">Skip round</button>
        </div>
