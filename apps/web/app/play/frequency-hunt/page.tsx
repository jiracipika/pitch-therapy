...
export default function FrequencyHuntPage() {
  ...
  const [results, setResults] = useState<
    { round: number; target: number; guess: number; diff: number; points: number }[]
  >([]);
  ...
  useEffect(() => {
    ...
    if (!recordedRef.current) {
      ...
      recordResult({
        ...
        score: score,
        accuracy:
          results.length > 0 ? results.filter((r) => r.diff <= 25).length / results.length : 0,
        rounds: totalRounds,
                ...
                <div>
                  ...
                  <div
                    style={{
                      ...
                      marginTop: 4,
                      color:
                        (results[results.length - 1]?.points ?? 0) > 500
                          ? "var(--ios-green)"
                          : (results[results.length - 1]?.points ?? 0) > 200
                            ? "var(--ios-orange)"
                            : "var(--ios-red)",
                    }}
                  >
