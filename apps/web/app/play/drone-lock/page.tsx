  ...
  useEffect(() => {
    ...
    if (!recordedRef.current) {
      ...
      recordResult({
        ...
        accuracy:
          results.length > 0
            ? results.filter((r) => Math.abs(r.cents) <= 20).length / results.length
            : 0,
        rounds: totalRounds,
  ...
  if (phase === "done") {
    return (
      <StudioResults
        ...
        headline="Drone locked."
        accent={ACCENT}
        stats={[
          { value: score, label: "SCORE", accentValue: true },
          { value: `${results.length ? Math.round(results.reduce((s, r) => s + Math.abs(r.cents), 0) / results.length) : 0}¢`, label: "AVG ERROR" },
        ...
        {(phase === "listening" || phase === "locking" || phase === "scored") && (
          <div style={{ textAlign: "center" }}>
            ...
            >
              ...
              <span style={{ fontSize: 13, color: "var(--ios-label3)" }}>
                Drone:{" "}
                <span style={{ color: "var(--ios-label)", fontWeight: 600 }}>
                  {NOTE_NAMES[droneNote]}4
                </span>
              </span>
            </div>
            ...
            <motion.button
              onClick={() =>
                playTone(NOTE_FREQS[droneNote] * Math.pow(2, targetInterval.semitones / 12), 1.0)
              }
              whileTap={{ scale: 0.92 }}
              style={{
                display: "inline-flex",
            ...
            <div style={{ marginBottom: 20 }}>
              ...
              >
                <span>-100¢</span>
                <span>-50¢</span>
                <span style={{ color: "var(--ios-green)" }}>0¢</span>
                <span>+50¢</span>
                <span>+100¢</span>
              </div>
              <div
