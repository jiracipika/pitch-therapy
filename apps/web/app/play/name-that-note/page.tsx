...
(NOTE_NAMES as unknown as string[]).forEach((n) => {
  NOTE_FREQS[`${n}4`] = NOTE_FREQUENCIES[`${n}4`] ?? 261.63;
});
const ACCENT = "#0A84FF";
...
export default function NameThatNotePage() {
  ...
  const handleAnswer = (noteName: string) => {
    ...
    setResults((r) => [
      ...r,
      { round, correct, points, target: targetNote.name, answer: noteName },
    ]);
  ...
  if (phase === "done") {
    return (
      <StudioResults
        ...
        headline="Named and shamed."
        accent={ACCENT}
        stats={[
          { value: score, label: "SCORE", accentValue: true },
          { value: `${results.filter((r) => r.correct).length}/${totalRounds}`, label: "CORRECT" },
          ...
          <div style={{ textAlign: "center" }}>
            ...
            {isTimed && !isPractice && (
              <div style={{ marginBottom: 16 }}>
                <div style={{ fontSize: 12, color: "var(--ios-label3)", marginBottom: 6 }}>
                  Time: {timeLeft}s
                </div>
                <div className="ios-progress-track">
                  <motion.div
                    ...
                    style={{
                      background: timeLeft <= 3 ? "var(--ios-red)" : ACCENT,
            ...
            <div style={{ fontSize: 12, color: "var(--ios-label3)", marginBottom: 16 }}>
