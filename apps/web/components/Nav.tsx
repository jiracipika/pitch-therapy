...
/* ── SF Symbol–equivalent SVG icons — clean, weight-matched ── */
...
function IconModes({ filled }: { filled: boolean }) {
  return (
...
const TABS = [
  ...
  { href: '/play-modes', label: 'Modes',   Icon: IconModes },
  ...
  const renderItem = (
    ...
    return (
      <Link
        key={href}
        href={href}
        className={`${rail ? 'pt-rail-item' : 'ios-tab-item'} ${active ? 'is-active' : ''}`}
        style={{ textDecoration: 'none', color }}
        aria-current={active ? 'page' : undefined}
      >
        <span style={{ color, transition: 'color 0.18s ease' }}>
          <Icon filled={active} />
        </span>
        <span
          style={{
