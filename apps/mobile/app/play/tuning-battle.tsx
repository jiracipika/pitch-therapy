...
export default function TuningBattleScreen() {
  ...
  const firstLockRef = useRef<number | null>(null);
  const judgmentTimeoutRef = useRef<ReturnType<typeof setTimeout> | null>(null);
  ...
  const startRound = useCallback(() => {
    ...
    playTone(note, NOTE_FREQS_4[note] || 440, 0.8);
  }, []);
  ...
  const selectNote = useCallback((pIdx: number, note: string) => {
    ...
    if (playersRef.current[pIdx].lockedIn) return;
  ...
  const judgeRound = useCallback(() => {
    ...
    const [playerOne, playerTwo] = playersRef.current;
    const playerOneCorrect = playerOne.selectedNote === targetRef.current;
    const playerTwoCorrect = playerTwo.selectedNote === targetRef.current;
    const winner = playerOneCorrect && playerTwoCorrect
      ? firstLockRef.current === 0 ? 'Player 1' : 'Player 2'
      : playerOneCorrect ? 'Player 1'
      : playerTwoCorrect ? 'Player 2'
      : null;
    ...
    if (winner) {
      setPlayers(current => {
        ...
        const winnerIndex = winner === 'Player 1' ? 0 : 1;
        next[winnerIndex] = { ...next[winnerIndex], score: next[winnerIndex].score + 1 };
        return next;
      });
    }
  ...
  const lockIn = useCallback((pIdx: number) => {
    ...
    const updatedPlayers = [...playersRef.current] as [Player, Player];
    updatedPlayers[pIdx] = { ...updatedPlayers[pIdx], lockedIn: true };
    if (firstLockRef.current === null) firstLockRef.current = pIdx;
    ...
    if (updatedPlayers[1 - pIdx].lockedIn) {
      judgeRound();
    } else {
      judgmentTimeoutRef.current = setTimeout(judgeRound, 2000);
    }
  }, [phase, judgeRound]);
  ...
  return (
    <View style={{ flex: 1, backgroundColor: pc.screen }}>
      ...
      <View style={{ alignItems: 'center', marginTop: 20 }}>
        <Text style={{ color: pc.textSecondary, fontSize: 14 }}>Listen, then select the note you hear</Text>
        <Pressable accessibilityRole="button" accessibilityLabel="Replay target note" onPress={() => playTone(targetNote, NOTE_FREQS_4[targetNote] || 440, 0.5)} style={{ marginTop: 8 }}>
