    ...
    return (
      <View style={{ flex: 1, backgroundColor: '#09090b' }}>
        <View style={{ paddingTop: 56, paddingHorizontal: 20, paddingBottom: 20 }}>
          ...
          <Text style={{ color: '#f4f4f5', fontSize: 22, fontWeight: '700', marginTop: 12 }}>Cents Deviation</Text>
          <Text style={{ color: '#71717a', fontSize: 14, marginTop: 4 }}>Detect microtonal shifts</Text>
        </View>
        <View style={{ flex: 1, paddingHorizontal: 20, justifyContent: 'center' }}>
          <Text style={{ color: '#f4f4f5', fontSize: 18, fontWeight: '600', marginBottom: 16 }}>Difficulty</Text>
          {(Object.keys(DIFF_CONFIG) as Difficulty[]).map(d => (
            <Pressable key={d} onPress={() => startGame(d)} style={({ pressed }) => ({ backgroundColor: 'rgba(255,255,255,0.04)', borderRadius: 16, padding: 20, borderWidth: 1, borderColor: 'rgba(255,255,255,0.07)', marginBottom: 12, opacity: pressed ? 0.75 : 1 })}>
              <Text style={{ color: '#f4f4f5', fontWeight: '600', fontSize: 16, textTransform: 'capitalize' }}>{d}</Text>
              <Text style={{ color: '#71717a', fontSize: 13, marginTop: 3 }}>±{DIFF_CONFIG[d].centsRange}¢ · {DIFF_CONFIG[d].rounds} rounds</Text>
            </Pressable>
          ))}
        </View>
      </View>
    ...
    return (
      <View style={{ flex: 1, backgroundColor: '#09090b' }}>
        <ScrollView contentContainerStyle={{ paddingTop: 80, paddingHorizontal: 20, paddingBottom: 40 }}>
          <Text style={{ color: '#f4f4f5', fontSize: 28, fontWeight: '700', marginBottom: 4 }}>Cents Deviation Complete!</Text>
  ...
  return (
    <View style={{ flex: 1, backgroundColor: '#09090b' }}>
      ...
      <View style={{ flex: 1, paddingHorizontal: 20, paddingTop: 32 }}>
        <Text style={{ textAlign: 'center', color: '#a1a1aa', fontSize: 14, marginBottom: 4 }}>Reference: <Text style={{ color: '#f4f4f5', fontWeight: '700' }}>{baseNote}</Text></Text>
        <Text style={{ textAlign: 'center', color: '#52525b', fontSize: 12, marginBottom: 16 }}>Listen then set the needle</Text>

        <View style={{ flexDirection: 'row', justifyContent: 'center', gap: 12, marginBottom: 24 }}>
          <Pressable onPress={() => playFrequency(baseFreq, 0.8)} style={{ width: 56, height: 56, borderRadius: 14, backgroundColor: 'rgba(255,255,255,0.04)', borderWidth: 1, borderColor: 'rgba(255,255,255,0.08)', alignItems: 'center', justifyContent: 'center' }}>
        ...
        <View style={{ marginBottom: 8 }}>
          <View style={{ flexDirection: 'row', justifyContent: 'space-between', marginBottom: 4 }}>
            <Text style={{ color: '#52525b', fontSize: 10 }}>-{CENTS_RANGE}¢</Text>
            <Text style={{ color: '#52525b', fontSize: 10 }}>0</Text>
            <Text style={{ color: '#52525b', fontSize: 10 }}>+{CENTS_RANGE}¢</Text>
          </View>
          <View style={{ height: 64, backgroundColor: 'rgba(255,255,255,0.04)', borderRadius: 16, position: 'relative', overflow: 'hidden', borderWidth: 1, borderColor: 'rgba(255,255,255,0.08)' }}>
        ...
        <View style={{ flexDirection: 'row', gap: 8, justifyContent: 'center', marginBottom: 16 }}>
          {[10, 5, 1].map(amt => (
            <View key={amt} style={{ flexDirection: 'row', gap: 4 }}>
              <Pressable onPress={() => adjustNeedle(-amt)} style={{ width: 44, height: 36, borderRadius: 8, backgroundColor: 'rgba(255,255,255,0.04)', borderWidth: 1, borderColor: 'rgba(255,255,255,0.07)', alignItems: 'center', justifyContent: 'center' }}>
        ...
        {submitted && (
          <View style={{ backgroundColor: 'rgba(132,204,22,0.08)', borderRadius: 12, padding: 14, marginBottom: 16, borderWidth: 1, borderColor: 'rgba(132,204,22,0.2)', alignItems: 'center' }}>
            <Text style={{ color: '#71717a', fontSize: 13 }}>Actual: <Text style={{ color: '#f4f4f5', fontWeight: '700' }}>{actualCents > 0 ? '+' : ''}{actualCents}¢</Text></Text>
            <Text style={{ color: '#71717a', fontSize: 13 }}>Guess: <Text style={{ color: '#f4f4f5', fontWeight: '700' }}>{needleCents > 0 ? '+' : ''}{needleCents}¢</Text></Text>
          </View>
        )}
